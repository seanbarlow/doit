...
def test_checkin_creates_commits(git_repo):
    ...
    result = subprocess.run(
        ["git", "commit", "-m", "Test commit"], cwd=git_repo, capture_output=True, text=True
    )
...
def test_checkin_git_operations_on_macos(git_repo, macos_test_env):
    ...
    result = subprocess.run(
        ["git", "checkout", "-b", "041-git-test"], cwd=git_repo, capture_output=True, text=True
    )
...
def test_checkin_branch_management(git_repo):
    ...
    status_result = subprocess.run(["git", "status"], cwd=git_repo, capture_output=True, text=True)
    ...
    for branch in branches:
        create_result = subprocess.run(
            ["git", "checkout", "-b", branch], cwd=git_repo, capture_output=True, text=True
        )
    ...
    if not result.stdout:
        pytest.skip(
            "git branch returned empty output - CI environment may have git configuration issues"
        )
