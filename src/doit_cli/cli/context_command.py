...
def show_context(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show full content of each source"),
) -> None:
    ...
    if config.summarization.enabled:
        soft_threshold = int(
            config.total_max_tokens * (config.summarization.threshold_percentage / 100.0)
        )
        console.print("\n[bold]Summarization:[/bold]")
        console.print("  Enabled: [green]yes[/green]")
        console.print(
            f"  Soft threshold: {soft_threshold:,} tokens ({config.summarization.threshold_percentage}%)"
        )
        console.print(
            f"  Fallback to truncation: {'yes' if config.summarization.fallback_to_truncation else 'no'}"
        )
    ...
    for source in context.sources:
        ...
        if source.truncated:
            status = (
                f"[yellow]truncated ({source.original_tokens:,} -> {source.token_count:,})[/yellow]"
            )
...
def context_status() -> None:
    ...
    if specs_dir.exists():
        ...
        console.print("  [dim]\u2717[/dim] specs directory: [dim](not found)[/dim]")
    ...
    if branch:
        ...
        if feature:
            ...
            if spec_path.exists():
                ...
                console.print("  [dim]\u2717[/dim] Current spec: [dim](no matching spec)[/dim]")
        else:
            console.print("  [dim]Not a feature branch (pattern: NNN-feature-name)[/dim]")
    else:
        console.print("  [dim]Not in a git repository or git not available[/dim]")

    # Usage hints
    console.print("\n[dim]Use 'doit context show' to see loaded context[/dim]")
    console.print(
        "[dim]Use 'doit context show --command specit' to see command-specific context[/dim]"
    )
